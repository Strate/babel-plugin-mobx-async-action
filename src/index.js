export default function (babel) {
    ...
    function convertToFlow(path, {actionIdentifier, mobxNamespaceIdentifier}) {
        ...
        CallExpression(path) {
            ...
            if (isAction(node.callee, actionIdentifier, mobxNamespaceIdentifier)) {
                if (node.arguments.length === 1) {
                    convertToFlow(path.get('arguments.0'), {actionIdentifier, mobxNamespaceIdentifier})
                    path.skip();
                } else if (node.arguments.length === 2) {
                    convertToFlow(path.get('arguments.1'), {actionIdentifier, mobxNamespaceIdentifier})
                    path.skip();
                }
            }
        },
                        ...
                        if (cPath.isExpressionStatement()) {
                            ...
                            if (exprPath.isAssignmentExpression() && exprPath.get('operator').node === '=') {
                                ...
                                if (
                                    ...
                                    convertToFlow(rightPath.get('body'), {actionIdentifier, mobxNamespaceIdentifier})
                                }
                            }
                        }
                    })
                } else {
                    convertToFlow(path.get('body'), {actionIdentifier, mobxNamespaceIdentifier})
                }
                path.skip();
                ...
                for (const {expression} of path.node.decorators) {
                    if (
                        ...
                        if (t.isClassMethod(path.node)) {
                            ...
                            convertToFlow(path.get('value'), {actionIdentifier, mobxNamespaceIdentifier})
                            path.skip();
                        }
                    }
                }
